...
export async function GET() {
  try {
    ...
    const [guestsRes, uploadsRes, eventsRes] = await Promise.all([
      ...
      supabaseAdmin.from("uploads").select("id, event_id, guest_id, file_name, file_type, file_size, drive_file_id, drive_view_url, thumbnail_url, is_featured, created_at").eq("wedding_id", weddingId!).order("created_at", { ascending: false }),
